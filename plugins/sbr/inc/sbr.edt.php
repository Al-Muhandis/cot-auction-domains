...
/*		cot_check(empty($rstagedays[$i]), $L['sbr_error_rstagedays'], 'rstagedays['.$i.']');
  ...
  $rsbr['sbr_taxpayer'] = $rsbrtaxpayer;  
    ...
    if(!empty($sbr['sbr_broker'])
		{
      $broker = $db->query("SELECT * FROM $db_users WHERE user_id=" . $sbr['sbr_broker'])->fetch();    
		  $rbody = cot_rc($L['sbr_mail_toperformer_edited_body'], array(
		  	'broker_name' => $broker['user_name'],
		  	'employer_name' => $usr['profile']['user_name'],
		  	'performer_name' => $performer['user_name'],        
	  		'sbr_title' => $rsbr['sbr_title'],
	  		'sbr_cost' => $rsbr['sbr_cost'].' '.$cfg['payments']['valuta'],	
      ...
      cot_sbr_sendpost($id, $L['sbr_posts_broker_edited'], $broker['user_id'], 0, 'info');      
    } 
    /* ===== */       
...
$t->assign(array(
	...
	'SBREDIT_FORM_MAINTITLE' => cot_inputbox('text', 'rsbrtitle', $sbr['sbr_title']), /* === Добавлено === */ 
