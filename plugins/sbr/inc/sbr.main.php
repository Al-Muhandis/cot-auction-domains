 ...
 * @package sbr
 * @version 1.0.0
 * @author CMSWorks Team, Suleymanov Renat
 * @copyright Copyright (c) CMSWorks, Suleymanov Renat
 * @license BSD
 */
...
// Действия только для Покупателя
if($usr['id'] == $sbr['sbr_employer'])
{
    ...
    switch ($rsbrtaxpayer) 
    {
      case 0: $totalcost = $sbr['sbr_cost'] + $sbr['sbr_tax']; break;
      case 1: $totalcost = 0; break;
      case 2: $totalcost = $sbr['sbr_cost'] + $sbr['sbr_tax']/2; break;
      default: $totalcost = $sbr['sbr_cost'] + $sbr['sbr_tax']; break;
    }		
	...
	if(!empty($num) && $a == 'done' && $sbr['sbr_status'] == 'process')
	{
		...
		if($stage = $db->query("SELECT * FROM $db_sbr_stages WHERE stage_sid=" . $id . " AND stage_status='process' AND stage_num=" . $num)->fetch())
		{
			...
			{	
			  /* === Изменено === */
        $stagescount = $db->query("SELECT COUNT(*) FROM $db_sbr_stages WHERE stage_sid=" . $id)->fetch(); 
        $tax1 = max($rsbr['sbr_cost']*$cfg['plugin']['sbr']['tax']/100, 1000);       // Если меньше 1000 р. комиссия, то 1000 р.
        $tax2 = $tax1 / $stagescount;        
        switch ($sbr['sbr_taxpayer']) 
        {
          case 0: $payperformerwithtax = $stage['stage_cost']; break;
          case 1: $payperformerwithtax = $stage['stage_cost'] - $tax2; break;
          case 2: $payperformerwithtax = $stage['stage_cost'] - $tax2/2; break;
          default: $payperformerwithtax = $stage['stage_cost']; break;
        }     
			  /* ===== */        
				...
				if($db->insert($db_payments, $payinfo))
				{
					if(!empty($rtext))
					{
						cot_sbr_sendpost($id, $rtext, $sbr['sbr_performer'], $sbr['sbr_employer']);                      
					}
					...
					if(!$notstartedstages)
					{
        			  ...
        			  /* === Добавлено === */   
            $paybroker = $sbr['sbr_atax'];
            // Выплата Брокеру
				    $payinfo['pay_userid'] = $sbr['sbr_broker'];
				    $payinfo['pay_area'] = 'balance';
				    $payinfo['pay_code'] = 'sbr:'.$id;
				    $payinfo['pay_summ'] = $paybroker;
				    $payinfo['pay_cdate'] = $sys['now'];
				    $payinfo['pay_pdate'] = $sys['now'];
				    $payinfo['pay_adate'] = $sys['now'];
				    $payinfo['pay_status'] = 'done';
				    $payinfo['pay_desc'] = cot_rc($L['sbr_broker_done_payments_desc'], 
				    	array(
					  	'sbr_title' => $sbr['sbr_title']
					    )
			  	  );
            ...
            if(!empty($sbr['sbr_broker']))
						{
              cot_sbr_sendpost($id, $L['sbr_posts_broker_done'], $sbr['sbr_broker'], 0, 'success', true);
            }            
					}
	...
	if($stage = $db->query("SELECT * FROM $db_sbr_stages WHERE stage_sid=" . $id . " AND stage_status='claim' AND stage_num=" . $num)->fetch())
	{
		...
		if(!cot_error_found())
		{
			...
			if($db->update($db_sbr_claims, $rclaim, "claim_sid=".$id." AND claim_stage=".$num))
			{
				...
				if($db->update($db_sbr_stages, $rstage, "stage_sid=" . $id . " AND stage_num=" . $num))
				{						
					$payperformerwithtax = $payperformer - $payperformer*$cfg['plugin']['sbr']['tax']/100;
