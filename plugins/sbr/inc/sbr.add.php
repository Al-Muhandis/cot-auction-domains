...
if ($a == 'add')
{
	...
	/* === Добавлено === */
  $rsbrtaxpayer = cot_import('rsbrtaxpayer', 'P', 'INT'); //0 - 1 - 2 Платит комиссию сервиса Покупатель - Продавец - 50:50    
  $rsbratax = cot_import('rsbratax', 'P', 'NUM');
	/* ===== */	
  ...
  {   
	  if (empty($bid))
	  { 
	   	$rsbremployer = cot_import('rsbremployer', 'P', 'TXT', 100, TRUE);
  		$rsbr['sbr_employer'] = $db->query("SELECT user_id FROM $db_users WHERE user_name = ? LIMIT 1", array($rsbremployer))->fetchColumn();
  		if (empty($rsbr['sbr_employer'])) cot_error('sbr_error_rsbremployer', 'rsbremployer');
  		if ($rsbr['sbr_employer'] == $usr['id']) cot_error('sbr_error_rsbremployernotyou', 'rsbremployer');
  	}
  	else
  	{
  		$rsbr['sbr_employer'] = $bid;
  	}
  }
  else
...
/*		cot_check(empty($rstagedays[$i]), $L['sbr_error_rstagedays'], 'rstagedays['.$i.']');
		...
		/* === Добавлено === */
  $rsbr['sbr_atax'] = $rsbratax;
  $rsbr['sbr_cost'] += $rsbratax;
		/* ===== */
		/* === Изменено === */  // $taxpayer или $rsbrtaxpayer
  $rsbr['sbr_tax'] = max($rsbr['sbr_cost']*$cfg['plugin']['sbr']['tax']/100, $cfg['plugin']['sbr']['mintax']);       // Если меньше 1000 р. комиссия, то 1000 р.  
  $rsbr['sbr_taxpayer'] = $rsbrtaxpayer;
    /* ===== */  
  ...
  /* === Удалено === */
	...
	if (!cot_error_found())
	{
		...
		for($i = 1; $i <= $stagescount; $i++)
		{
			...
			$rstage['stage_days'] = 7;  // Изменено вместо rstagedays[$i]
    ...
    $employer = $db->query("SELECT * FROM $db_users WHERE user_id=" . $rsbr['sbr_employer'])->fetch();
		...
		$rbody = cot_rc($L['sbr_mail_toperformer_new_body'], array(
			...
			'employer_name' => $employer['user_name'],
...
/* === Добавлено ===*/
		$rbody = cot_rc($L['sbr_mail_toemployer_new_body'], array(
			'performer_name' => $performer['user_name'],
			'employer_name' => $employer['user_name'],
			'sbr_title' => $rsbr['sbr_title'],
			'sbr_cost' => $rsbr['sbr_cost'].' '.$cfg['payments']['valuta'],	
		...
		cot_mail ($employer['user_email'], $rsubject, $rbody);
		if ($rsbr['sbr_broker']>0) 
    { 
      $broker = $db->query("SELECT * FROM $db_users WHERE user_id=" . $rsbr['sbr_broker'])->fetch();
		  $rbody = cot_rc($L['sbr_mail_toperformer_new_body'], array(
		  	'performer_name' => $performer['user_name'],
		  	'employer_name' => $employer['user_name'],
			  'sbr_title' => $rsbr['sbr_title'],
      ...
      cot_mail($broker['user_email'], $rsubject, $rbody); 
    }
/* ===== */      

		cot_sbr_sendpost($id, $L['sbr_posts_performer_new'], $rsbr['sbr_performer'], 0, 'info');
		cot_sbr_sendpost($id, $L['sbr_posts_employer_new'], $rsbr['sbr_employer'], 0, 'info');      
    
		
/* === Изменено ===*/    
		if (($erole == 0) or ($erole == 1)) 
    { cot_redirect(cot_url('sbr', 'id=' . $id, '', true)); }
    else
    { cot_redirect(cot_url('sbr', '', '', true)); }
/* ===== */    
	}
}
...
if(!empty($bid))
{
	$t->assign(cot_generate_usertags($bid, 'SBR_EMPLOYER_'));
}
else
{
	$t->assign('SBRADD_FORM_EMPLOYER', cot_inputbox('text', 'rsbremployer', $rsbremployer, 'placeholder="'.$L['sbr_employer_placeholder'].'"'));
}
...
$t->assign(array(
	...
	'SBRADD_FORM_SEND' => cot_url('sbr', 'm=add&pid='.$pid.'&uid='.$uid.'&bid='.$bid.'&role='.$erole.'&a=add'),
	'SBRADD_FORM_OWNER' => cot_build_user($usr['id'], htmlspecialchars($usr['name'])),
	'SBRADD_FORM_OWNERID' => $usr['id'],
	'SBRADD_FORM_MAINTITLE' => cot_inputbox('text', 'rsbrtitle', $rsbr['sbr_title']), /* === Добавлено === */
  'SBRADD_FORM_ATAX' => cot_inputbox('text', 'rsbratax', $rsbr['sbr_atax'], array('class' => 'stagecost', 'size' => '10', 'maxlength' => '100')), 
  'SBRADD_FORM_TAXPAYER' => cot_radiobox('0', 'rsbrtaxpayer', array('0', '1', '2'), array('Покупатель', 'Продавец', '50:50')), /* ===== */
));
...
for($i = 1; $i <= $stagescount; $i++)
{
	$t->assign(array(
		'STAGEADD_FORM_NUM' => $i,
		'STAGEADD_FORM_TITLE' => cot_inputbox('text', 'rstagetitle['.$i.']', $rstagetitle[$i], 'placeholder="sample.com"'),
		'STAGEADD_FORM_TEXT' => cot_textarea('rstagetext['.$i.']', $rstagetext[$i], 10, 120, 'placeholder="Особые требования или условия передачи домена, контактные данные или профиль у регистратора нового владельца"', 'input_textarea'),
