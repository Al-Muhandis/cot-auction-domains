		...
		<form action="{SBRADD_FORM_SEND}" enctype="multipart/form-data" method="post" name="sbrform" id="sbrform">
          ...
          <!-- IF {PHP.erole} != 0 -->          
					<tr>
						...
						<td class="width70">
							<!-- IF {PHP.bid} -->
							<a href="{SBR_EMPLOYER_DETAILSLINK}" target="blank">{SBR_EMPLOYER_NICKNAME}</a>
							<!-- ELSE -->
							{SBRADD_FORM_EMPLOYER}
							<!-- ENDIF -->
						</td>
					</tr>
					<!-- ENDIF -->                    
			...
			<div id="sbrstageslist">
				...
				<div class="stageblock<!-- IF {STAGEADD_FORM_NUM} == {PHP.stagescount} --> laststage<!-- ENDIF --> well">
					...
					<table class="table customform">
						<tr>
							<td class="width30"><b>{PHP.L.sbr_stage} № <span class="stagenum">{STAGEADD_FORM_NUM}</span></b></td>
          ...
          <!-- IF {PHP.erole} == 2 -->        
					<tr>
						<td class="width30"><b>Ваша агентская комиссия ({PHP.cfg.payments.valuta})</b></td>
						<td class="width70">{SBRADD_FORM_ATAX}</td>
					</tr>
          <!-- ENDIF -->
					<tr>
						<td class="width30"><b>Комиссию сервиса платит </b></td>
						<td class="width70">{SBRADD_FORM_TAXPAYER}</td>
					</tr>          
				</table>
			</div>
...
<script>
	...
	$().ready(function() {
      ...
      var taxpayer = parseInt($('#sbrform input:radio:checked').val()); 
      ...
      switch (taxpayer) {
        case 0: 
          ...
          break
        case 1: 
          $('#total').html(stagescost)
          break
        case 2:
          $('#total').html(stagescost + taxsumm/2)
          break
      }         
		});
	});
